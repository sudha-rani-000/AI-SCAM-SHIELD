/**
 * A lightweight, fully client-side heuristic scanner. It looks for the
 * language patterns most common in phishing/scam messages and produces
 * a weighted score. This is a real (if simple) rule-based detector —
 * not a placeholder — but it's not a substitute for a trained model or
 * a security team; treat its output as a first-pass signal.
 */
...
const INDICATORS = [
  {
    ...
    pattern: /\b(act now|immediately|urgent(ly)?|right away|within\s+\d+\s*(hours?|minutes?|days?)|expires?\s+(today|soon)|last chance|final notice)\b/i,
    weight: 15,
  ...
  },
  {
    ...
    pattern: /\b(gift card|itunes card|wire transfer|bitcoin|crypto(currency)?|western union|money ?gram|prepaid (card|debit))\b/i,
    weight: 28,
  ...
  },
  {
    ...
    pattern: /\b(verify your account|confirm your password|log ?in to your account|update your (payment|billing) (info|details)|social security number|ssn|routing number|account number|one[- ]time (code|password)|otp)\b/i,
    weight: 26,
  ...
  },
  {
    ...
    pattern: /\b(you('| ha)ve won|congratulations,? you|claim your prize|free (iphone|gift|money|reward)|lottery|you('re| are) selected)\b/i,
    weight: 22,
  ...
  },
  {
    ...
    pattern: /\b(account (will be |has been )?(suspended|closed|terminated|locked)|legal action|arrest warrant|irs|tax debt|court (order|summons)|failure to (comply|respond))\b/i,
    weight: 22,
  ...
  },
  {
    ...
    pattern: /\b(paypal|amazon|apple|microsoft|netflix|bank of america|wells fargo|chase|irs|social security administration)\b/i,
    weight: 10,
...
export function scanText(rawText) {
  ...
  // Confidence grows with both the number of independent signals found
  // and the amount of text available to search — a one-line message
  // with no hits is a weaker "safe" signal than a full paragraph with none.
